...
* Info-------------------------------------------------------------------------------
** File name:			GLCD.c
** Descriptions:		Has been tested
* SSD1289��ILI9320��R61505U��SSD1298��ST7781��SPFD5408B��ILI9325��ILI9328�� *
*					HX8346A��HX8347A
**------------------------------------------------------------------------------------------------------
** Created by:			AVRman
...
static void LCD_Configuration(void) {
  ...
  /* EN = P0.19 , LE = P0.20 , DIR = P0.21 , CS = P0.22 , RS = P0.23 , RS =
   * P0.23 */
  /* RS = P0.23 , WR = P0.24 , RD = P0.25 , DB[0.7] = P2.0...P2.7 , DB[8.15]=
   * P2.0...P2.7 */
 ...
 * Attention		 : None
 *******************************************************************************/
static __attribute__((always_inline)) void LCD_WriteReg(uint16_t LCD_Reg,
...
static void LCD_SetCursor(uint16_t Xpos, uint16_t Ypos) {
  ...
  switch (LCD_Code) {
  default: /* 0x9320 0x9325 0x9328 0x9331 0x5408 0x1505 0x0505 0x7783 0x4531
              0x4535 */
...
/***************************************************************CUSTOM
 * FUNCTIONS*************************/
void LCD_FillRect(uint16_t Xpos, uint16_t Ypos, uint16_t w, uint16_t h,
                  uint16_t color) {
  uint16_t i, j;

  for (i = 0; i < h; i++) {
    LCD_SetCursor(Xpos, Ypos + i); // Sets the cursor to start of line

    LCD_WriteIndex(0x0022); // Used to start writing on the GRAM reg again
...
void LCD_EmptyRect(uint16_t Xpos, uint16_t Ypos, uint16_t w, uint16_t h,
                   uint16_t color, uint8_t thickness) {
  LCD_FillRect(Xpos, Ypos, thickness, h, color); // Vertical left line
  LCD_FillRect(Xpos, Ypos, w, thickness, color); // Horizontal top line
  LCD_FillRect(Xpos + w - thickness, Ypos, thickness, h,
               color); // Vertical right line
  LCD_FillRect(Xpos, Ypos + h - thickness, w, thickness,
               color); // Horizontal bottom line
}
